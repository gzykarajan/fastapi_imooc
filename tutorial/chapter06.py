...
from fastapi.security import OAuth2PasswordBearer, OAuth2PasswordRequestForm  # 表单
...
fake_users_db = {
    "john snow": {
        ...
        "hashed_password": "fakehashedsecret",  # 加密后的密码，不是明文密码
        "disabled": False,
    },
    "alice": {
        "username": "alice",
...
class User(BaseModel):
    ...
    disabled: Optional[bool] = None  # 用这个字段区分活跃和不活跃账户
...
async def get_current_active_user(current_user: User = Depends(get_current_user)):  # get_current_user不能带（）！
...
# linux下生成密钥用这个命令 openssl rand -hex 32
ALGORITHM = "HS256"  # 算法
...
def verity_password(plain_password: str, hashed_password: str):
    """对密码进行校验"""
    """verify() 接收明文密码和加密的密码，返回值是True或False"""
...
# 有了密码校验功能 和 用户 就可以对用户进行校验了
def jwt_authenticate_user(db, username: str, password: str):
    user = jwt_get_user(db, username=username)
    if not user:
        return False
...
@app06.post("/jw/token", response_model=Token)
async def login_for_access_token(form_data: OAuth2PasswordRequestForm=Depends()):
    # 接收表单数据，数据为名户名和密码，先校验，不通过返回401校验码
    # 通过则更新token更新时间，再把token创建好
